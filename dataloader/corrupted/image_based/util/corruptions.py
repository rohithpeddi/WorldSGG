...
from dataloader.corrupted.image_based.util.corruption_utils import disk, MotionImage, clipped_zoom, plasma_fractal
...
class FixedImageCorruptionGenerator(BaseImageCorruptionGenerator):

    def __init__(self, image_corruption_type, corruption_severity_level):
        super().__init__()
        self._image_corruption_type = image_corruption_type
        self._corruption_severity_level = corruption_severity_level

    def fetch_corruption_transforms(self, num_frames):
        if self._image_corruption_type is None:
            corruption_index = np.random.randint(0, len(self._corruptions_list))
            self._image_corruption_type = self._corruptions_list[corruption_index]

        corruption_transform_list = []
        for _ in range(num_frames):
            corruption_transform = self._image_corruption_type if np.random.rand() < self._no_corruption_threshold else const.NO_CORRUPTION
            corruption_transform_list.append(self._corruption_name_to_function[corruption_transform])

        return corruption_transform_list

class ContinuousImageCorruptionGenerator(BaseImageCorruptionGenerator):
    ...
    def fetch_corruption_transforms(self, num_frames):
        corruption_transform_list = []
        # For each frame we apply a set of corruptions each at a different corruption level
        # We randomly select the number of corruptions to apply
        num_corruptions = np.random.randint(3, 7)

        image_corruption_list = []
        for _ in range(num_corruptions):
            corruption_index = np.random.randint(0, len(self._corruptions_list))
            image_corruption_type = self._corruptions_list[corruption_index]
            corruption_transform_function = self._corruption_name_to_function[image_corruption_type]
            if self._mode == const.PREDCLS:
                severity_level = 5
            else:
                # Randomly select a severity level between 2 and 4
                severity_level = np.random.randint(1, 5)
            image_corruption_list.append((corruption_transform_function, severity_level))

        for _ in range(num_frames):
            # Apply the same set of corruptions to all the frames of the videos
            corruption_transform_list.append(image_corruption_list)
        return corruption_transform_list
