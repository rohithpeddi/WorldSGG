...
Camera-relative positions are computed from the extrinsic matrix $\mat{T} = [\mat{R}\,|\,\mat{t}]$. A global camera token $\mat{c}_{\text{cam}}$ is formed from $\op{MLP}([\op{6D}(\mat{R}) \;\|\; \mat{t}])$. For each object, the direction $\mat{d}_n = \bar{\mat{c}}_n - \mat{t}$, view alignment $\alpha_n$, and azimuth $\beta_n$ are projected:
\begin{equation}
...
\begin{align}
    ...
    \mat{t}_{\text{rel}}^{(t)} &= \mat{t}_t - \mat{R}_{\text{rel}}^{(t)}\mat{t}_{t-1}. \label{eq:rel_trans}
\end{align}
