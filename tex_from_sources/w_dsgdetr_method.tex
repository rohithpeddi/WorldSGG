...
\begin{equation}
    \tilde{\mat{X}}_n = \op{TransformerEncoder}\bigl(\{\mat{x}_n^{(t)} + \mat{PE}_{\text{sin}}(t)\}_{t=1}^T,\; \text{mask}=\text{pad\_mask}_n\bigr),
    \label{eq:toe}
\end{equation}
...
The temporally-refined tokens $\tilde{\mat{X}}^{(t)}$ are passed through the \emph{InterObjectTransformer} for intra-frame spatial reasoning. Additive pairwise spatial positional embeddings $\mat{S}^{(t)}$ provide 3D relational biases:
\begin{equation}
...
\State $\tilde{\mat{X}} \gets \text{TemporalObjectEncoder}(\mat{X})$ \Comment{Explicit temporal tracking}
