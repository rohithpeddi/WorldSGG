...
# Suppress the spammy "truncation" warning from transformers that fires
# on every apply_chat_template call and floods logs via vLLM's warning handler
os.environ["TRANSFORMERS_NO_ADVISORY_WARNINGS"] = "1"
warnings.filterwarnings("ignore", message=".*intended overrides.*truncation.*")
warnings.filterwarnings("ignore", message=".*not keyword args.*truncation.*")
...
class InternVLModel(BaseVideoModel):
    ...
    def _prepare_frames(self, video_inputs):
        """Extract, subsample, and resize PIL frames from video tensor."""
        ...
        # Limit resolution to prevent massive dynamic patching context explosion
    ...
    # _tensor_to_pil_frames is inherited from BaseVideoModel
    ...
    def mllm_batch_response(self, prompts):
        ...
        tokenizer = self.model.get_tokenizer()
        # Cache frames per unique tensor to avoid redundant tensor→PIL→resize
        frame_cache = {}
        prompt_inputs = []
        max_tokens_list = []
        skipped = set()
