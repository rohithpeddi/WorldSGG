...
class AgDataBBAnnotations:
    ...
    def __init__(
        ...
        # Load annotation paths from config if available, otherwise
        # fall back to ag_root-relative defaults.
        if cfg is None:
        ...
        ann_cfg = cfg.get("paths", {}).get("annotations", {})

        def _resolve(cfg_key: str, *relative_parts: str) -> Path:
            """Return the config path if set, else ag_root / relative_parts."""
            val = ann_cfg.get(cfg_key, "")
            if val:
                return Path(val)
            return self.ag_root_directory.joinpath(*relative_parts)
        ...
        self.dynamic_detections_root_path = _resolve(
            "detection_dynamic", "detection", "gdino_bboxes",
        )
        self.static_detections_root_path = _resolve(
            "detection_static", "detection", "gdino_bboxes_static",
        )

        # Annotated frame and sampling info
        self.frame_annotated_dir_path = self.ag_root_directory / "frames_annotated"
        self.sampled_frames_idx_root_dir = _resolve(
            "sampled_frames_idx", "sampled_frames_idx",
        )

        # GT annotations
        self.gt_annotations_root_dir = _resolve(
        ...
        world_ann = _resolve("world_annotations", "world_annotations")
        self.world_annotations_root_dir = world_ann
        self.bbox_3d_root_dir = world_ann / "bbox_annotations_3d"
        self.bbox_4d_root_dir = world_ann / "bbox_annotations_4d"
        self.bbox_3d_final_root_dir = world_ann / "bbox_annotations_3d_final"
        self.bbox_3d_gt_root_dir = world_ann / "bbox_annotations_3d_gt"
        self.bbox_3d_gt_obb_root_dir = world_ann / "bbox_annotations_3d_obb"
        # Critical path: this is where get_final_data_lite loads from
        self.bbox_3d_gt_obb_final_root_dir = _resolve(
            "bbox_3d_gt_obb_final",
