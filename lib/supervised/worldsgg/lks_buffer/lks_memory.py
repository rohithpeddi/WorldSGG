...
The core mechanic of Baseline 1: "objects freeze when the camera looks away."

Update rule (hard-coded, NO neural parameters):
  - Visible (i ∈ FOV):  M[i] ← V_i^t          (overwrite with fresh features)
  - Unseen  (j ∉ FOV):  M[j] ← M[j]           (carry forward unchanged)
...
class LKSMemoryBuffer:
    ...
    Represents a "sticky map" / "semantic SLAM" approach: the world
    is assumed to freeze the moment the camera looks away.
    ...
    def __init__(self, max_objects: int, d_visual: int, device: str = "cpu"):
    ...
    def update(
        ...
        """
        Zero-order hold update. All operations are detached.
    ...
    def get_features(self, N: int = None) -> torch.Tensor:
        ...
        Returns:
            (N, d_visual) — detached visual features.
                Fresh for recently visible objects.
                Stale for objects seen in the past but currently unseen.
                Zeros for never-seen objects.
        """
        if N is None:
            N = self.max_objects
