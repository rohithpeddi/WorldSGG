...
\begin{enumerate}
    ...
    \item \textbf{3D IoU (AABB)}: Intersection-over-union computed on axis-aligned bounding boxes (AABB) derived from the 8 corners. While not oriented IoU, this metric is numerically stable and interpretable.  We report $\text{mAP}_{\text{3D}}^{50}$ (fraction of matches with 3D IoU $\geq 0.5$) and $\text{mAP}_{\text{3D}}^{75}$.
