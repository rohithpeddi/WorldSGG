...
def rerun_frame_vis_camera(
    video_id: str,
    *,
    frames_final: Dict[str, Any],
    frame_annotated_dir_path: Path,
    img_maxsize: int = 320,
    app_id: str = "World4D-CameraFrame",
    ...
    for vis_t in range(S):
        ...
        # BBoxes (already in camera frame)
    ...
    print(f"[camera-frame-vis] running for {video_id}. Scrub the 'frame' timeline in Rerun.")
...
class FrameToWorldAnnotations(FrameToWorldAnnotationsBase):
    ...
    def visualize_camera_frame(self, video_id: str, *, app_id: str = "World4D-CameraFrame") -> None:
        """
        Visualize camera-frame data (points, bboxes in camera coordinate frame).
        Loads from bbox_3d_camera_root_dir.
        """
        camera_pkl = self.bbox_3d_camera_root_dir / f"{video_id[:-4]}.pkl"
        if not camera_pkl.exists():
            raise FileNotFoundError(f"Camera-frame PKL not found: {camera_pkl}")
        ...
        if frames_final is None:
            raise ValueError(f"[camera-frame][{video_id}] frames_final missing in {camera_pkl}")

        rerun_frame_vis_camera(
            video_id,
...
def main_sample():
    """
    Simple entry point to visualize camera-frame point clouds
    ...
    frame_to_world_generator.visualize_camera_frame(video_id=video_id, app_id="World4D-CameraFrame-Sample")
