...
def visualize_with_rerun(predictions: dict,
                         ...
                         spawn: bool = True):
    """
    Animate scenes with rerun (one time-step per frame).
    """
    try:
        import rerun as rr
    except ImportError:
        print("[INFO] rerun-sdk not installed. Skipping visualization.")
        return

    rr.init("AG-Pi3 Scenes", spawn=spawn)

    # World coordinates convention (optional, but helps)
    try:
    ...
    if log_cameras and "camera_poses" in predictions:
        ...
        for i in range(len(cam_poses)):
            ...
            rr.log(f"world/cameras/cam_{i}",
                   rr.Transform3D(rotation=rr.Rotation3D(mat3x3=R),
                                  translation=t),
                   timeless=True)
    ...
    for i, (P, C) in enumerate(zip(per_frame_points, per_frame_colors)):
        ...
        rr.log("world/frame/merged_points",
               rr.Points3D(positions=P.astype(np.float32),
