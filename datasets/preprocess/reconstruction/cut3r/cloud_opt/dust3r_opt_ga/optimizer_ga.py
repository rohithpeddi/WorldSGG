...
from dust3r.utils.goem_opt import DepthBasedWarping, OccMask, WarpImage, depth_regularization_si_weighted, \
	tum_to_pose_matrix
from tqdm import tqdm

from cloud_opt.dust3r_opt.commons import edge_str
...
class PointCloudOptimizer(BasePCOptimizer):
	...
	def __init__(self, *args, optimize_pp=False, focal_break=20, shared_focal=False, flow_loss_fn='smooth_l1',
	             ...
	             sintel_ckpt=False, use_self_mask=False, pxl_thre=50, sam2_mask_refine=True, motion_mask_thre=0.35,
	             batchify=False,
		...
		if self.flow_loss_weight > 0:
			self.flow_ij, self.flow_ji, self.flow_valid_mask_i, self.flow_valid_mask_j = self.get_flow(
				sintel_ckpt)  # (num_pairs, 2, H, W)
	...
	def load_window_params(self, window_idx):
		...
		if window_idx == 0 and self.prev_video_results is None:
			self.opt_im_depthmaps = ParameterStack(self.im_depthmaps[win_start:win_end], is_param=True,
			                                       fill=self.max_area).requires_grad_(True)
			...
			self.frozen_im_depthmaps = ParameterStack(self.im_depthmaps[win_start:opt_start], is_param=True,
			                                          fill=self.max_area).requires_grad_(False)
			self.opt_im_depthmaps = ParameterStack(self.im_depthmaps[opt_start:win_end], is_param=True,
			                                       fill=self.max_area).requires_grad_(True)
			self.frozen_im_poses = ParameterStack(self.im_poses[win_start:opt_start], is_param=True).requires_grad_(
				False)
		...
		self.register_buffer('_win_pp',
		                     torch.tensor([(w / 2, h / 2) for h, w in self.win_imshapes], device=self.device))
		...
		self.win_total_area_i = sum(
			[self.im_areas[i] for i, j in torch.tensor(self.edges, device=self.device)[valid_edges]])
		self.win_total_area_j = sum(
			[self.im_areas[j] for i, j in torch.tensor(self.edges, device=self.device)[valid_edges]])
	...
	def get_flow(self, sintel_ckpt=False):  # TODO: test with gt flow
		...
		flow_net = load_RAFT() if sintel_ckpt else load_RAFT(
	...
	def get_motion_mask_from_pairs(self, view1, view2, pred1, pred2):
		...
		ego_flow_1_2, _ = self.depth_wrapper(self.R_i, self.T_i, self.R_j, self.T_j, 1 / (self.depth_maps_i + 1e-6),
		                                     self.intrinsics_j, torch.linalg.inv(self.intrinsics_i))
		ego_flow_2_1, _ = self.depth_wrapper(self.R_j, self.T_j, self.R_i, self.T_i, 1 / (self.depth_maps_j + 1e-6),
		                                     self.intrinsics_i, torch.linalg.inv(self.intrinsics_j))
	...
	def depth_to_pts3d_partial(self):
		...
		# get pointmaps in camera frame
		rel_ptmaps = [depthmap_to_pts3d(depth[i][None], focal_ex(i), pp=pp[i:i + 1])[0] for i in
		              range(im_poses.shape[0])]
		# project to world frame
	...
	def forward_batchify(self, epoch=9999):
		...
		if self.flow_loss_weight > 0 and epoch >= self.num_total_iter * self.flow_loss_start_epoch:  # enable flow loss after certain epoch
			...
			flow_loss_i = self.flow_loss_fn(ego_flow_1_2[:, :2, ...], self.flow_ij, ~dynamic_mask1,
			                                per_pixel_thre=self.pxl_thre)
			flow_loss_j = self.flow_loss_fn(ego_flow_2_1[:, :2, ...], self.flow_ji, ~dynamic_mask2,
			                                per_pixel_thre=self.pxl_thre)
	...
	def forward_window_wise(self, epoch=9999):
		...
		# compute the loss
		li = self.dist(proj_pts3d[self._win_ei], aligned_pred_i,
		               weight=self._win_weight_i).sum() / self.win_total_area_i
		lj = self.dist(proj_pts3d[self._win_ej], aligned_pred_j,
		               weight=self._win_weight_j).sum() / self.win_total_area_j
		
		# camera temporal loss
		if self.temporal_smoothing_weight > 0:
			temporal_smoothing_loss = self.relative_pose_loss(self.get_win_im_poses()[:-1],
			                                                  self.get_win_im_poses()[1:]).sum()
		else:
			temporal_smoothing_loss = 0
		...
		if self.flow_loss_weight > 0 and epoch >= self.num_total_iter * self.flow_loss_start_epoch:  # enable flow loss after certain epoch
			...
			flow_loss_i = self.flow_loss_fn(ego_flow_1_2[:, :2, ...], self.win_flow_ij, ~dynamic_mask1,
			                                per_pixel_thre=self.pxl_thre)
			flow_loss_j = self.flow_loss_fn(ego_flow_2_1[:, :2, ...], self.win_flow_ji, ~dynamic_mask2,
			                                per_pixel_thre=self.pxl_thre)
			flow_loss = flow_loss_i + flow_loss_j
