...
if __name__ == '__main__':
    ...
    videos = sorted(os.listdir(cfg['img_path']))
    BASE = cfg['img_path']
    output_dir = cfg['output']
    prompt = cfg['prompt']

    for video in videos:
        cfg['img_path'] = path.join(BASE, video, "rgb")
        cfg['output'] = path.join(BASE, video, output_dir)
        cfg['prompt'] = path.join(BASE, video, prompt)
