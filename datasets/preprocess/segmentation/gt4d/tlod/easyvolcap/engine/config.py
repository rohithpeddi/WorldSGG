...
class Config:
    ...
    def _substitute_predefined_vars(filename, temp_config_name):
        ...
        support_templates = dict(  # noqa: C408
    ...
    @staticmethod
    def _file2dict(filename, use_predefined_variables=True, extra_base_cfg_dict={}):  # noqa: C901, B006
        ...
        if BASE_KEY in cfg_dict:
            ...
            for f in base_filename:
                ...
                _cfg_dict, _cfg_text = Config._file2dict(
                    f,
                    extra_base_cfg_dict=Config._merge_a_into_b(
                        base_cfg_dict, deepcopy(extra_base_cfg_dict)
                    ),
                )

                # Merge base into current base
                base_cfg_dict = Config._merge_a_into_b(
                    _cfg_dict, base_cfg_dict
                )  # merge next base into previous base, and then merge all base into current
            ...
            cfg_dict = Config._substitute_base_vars(
                cfg_dict,
                base_var_dict,
                Config._merge_a_into_b(base_cfg_dict, deepcopy(extra_base_cfg_dict)),
            )
        else:
            cfg_dict = Config._substitute_base_vars(
                cfg_dict, base_var_dict, extra_base_cfg_dict
            )
    ...
    def _merge_a_into_b(a, b, allow_list_keys=True):
        ...
        if BASE_KEY in a:
            ...
            for f in base_filename:
                # NOTE: easyvolcap: use project-wise relative path for configuration?
                _cfg_dict, _cfg_text = Config._file2dict(
                    f, extra_base_cfg_dict=b
                )  # allow project level path
        ...
        for k, v in a.items():
            if allow_list_keys and k.isdigit() and isinstance(b, list):
                ...
                if k == len(
                    b
                ):  # MARK: Assuming all the keys come in sorted order to be inserted
                    b.append(v)
                elif k > len(b):
            ...
            if k == APPEND_KEY and isinstance(b, list):
                ...
                if ((k in b) or (isinstance(b, list) and k <= len(b))) and (
                    not delete_base
                ):  # dict? list? not delete?
    ...
    def fromstring(cfg_str, file_format):
        ...
        if file_format != ".py" and "dict(" in cfg_str:
            ...
            warnings.warn(
                'Please check "file_format", the file format may be .py', stacklevel=2
            )
    ...
    @property
    def pretty_text(self):  # noqa: C901
        indent = 4
...
class DictAction(Action):
    ...
    def __call__(self, parser, namespace, values, option_string=None):
        ...
        while i < len(values):
            if "=" in values[i]:
                key, val = values[i].split("=", maxsplit=1)
