    ...
    def merge_ret(ret, x: torch.Tensor, sh: torch.Size, nn_dim: int):
        ...
        if len(ret) and isinstance(ret[0], torch.Tensor):
            ...
            if ignore_mismatch:
                ...
                ret = (
                    ret.view(*sh, *ret.shape[nn_dim + 1 :])
                    if x.shape[nn_dim] == ret.shape[nn_dim]
                    else ret
                )
            ...
            # Start recursion
            ret = {
                k: merge_ret([v[k] for v in ret], x, sh, nn_dim) for k in ret[0].keys()
            }
            ret = dict_type(ret)
            ...
            # Start recursion
            ret = [
                merge_ret([v[i] for v in ret], x, sh, nn_dim)
                for i in range(len(ret[0]))
            ]
            ret = list_type(ret)
        else:
            raise RuntimeError(
                f"Unsupported return type to batchify: {type(ret[0])}, or got empty return value"
            )
        return ret
    ...
    def wrapper(decoder: Callable[[torch.Tensor], torch.Tensor]):
        def decode(*args, **kwargs):
            ...
            if merge_dims:
                ...
                tensor_args = [
                    v.view(v.shape[0], -1, *v.shape[nn_dim + 1 :]) for v in tensor_args
                ]
                tensor_kwargs = {
                    k: v.view(v.shape[0], -1, *v.shape[nn_dim + 1 :])
            ...
            actual_size = (
                math.ceil(total_size / math.ceil(total_size / chunk_size))
                if total_size
                else chunk_size
            )  # this value should be smaller than the actual chunk_size specified
            if print_progress:
                pbar = tqdm(total=total_size, back=3)  # log previous frame
            for i in range(0, total_size, actual_size):
                # nn_dim should be used if there's multiplication involved
                chunk_args = [
                    v[(slice(None),) * nn_dim + (slice(i, i + actual_size),)]
                    for v in tensor_args
                ]
                chunk_kwargs = {
                    k: v[(slice(None),) * nn_dim + (slice(i, i + actual_size),)]
                    for k, v in tensor_kwargs.items()
                }

                # Other components can use this to perform manual trunking
                if batch_key in other_kwargs:
                    other_kwargs[batch_key].meta[inds_key] = [i, i + actual_size]

                result = decoder(
                    *chunk_args, *other_args, **chunk_kwargs, **other_kwargs
                )
            ...
            if print_progress:
                pbar.close()  # manual close necessary!

            if not len(ret):
                # Brute-forcely go through the network with empty input
                log(
                    "zero length tensor detected in chunkify, are the camera parameters correct?",
                    "red",
                )
                i = 0
                chunk_args = [
...
def batch_aware_indexing(
    mask: torch.Tensor, metric: torch.Tensor = None, dim=-1
) -> Tuple[torch.Tensor, torch.Tensor, int]:  # MARK: SYNC
    # dim: in terms of the index (mask)
    if mask.dtype != torch.bool:
        mask = mask.bool()
    if metric is None:
        metric = mask.int()
    if metric.dtype == torch.bool:
        metric = metric.int()
    # retain all other dimensions (likely batch dimensions)
    S = (
        mask.sum(dim=dim).max().item()
    )  # the max value of this dim on all other dimension
    valid, inds = metric.topk(
        S, dim=dim, sorted=False
    )  # only find the top (mask = True) values (randomly select other values)
    return valid, inds, S
...
def multi_scatter(
    target: torch.Tensor, indices: torch.Tensor, values: torch.Tensor, dim=-2
):
...
def multi_gather_tris(v: torch.Tensor, f: torch.Tensor, dim=-2) -> torch.Tensor:
    ...
    return multi_gather(v, f.view(*f.shape[:-2], -1), dim=dim).view(
        *f.shape, *remainder
    )  # B, F, 3, 3
...
def linear_scatter(
    target: torch.Tensor, indices: torch.Tensor, values: torch.Tensor, dim=0
):
...
def scatter0(target: torch.Tensor, inds: torch.Tensor, value: torch.Tensor):
    return target.scatter(
        0, expand_at_the_back(target, inds), value
    )  # Surface, 3 -> B * S, 3
