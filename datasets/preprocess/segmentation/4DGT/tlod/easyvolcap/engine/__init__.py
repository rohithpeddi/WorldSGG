...
from .registry import Registry  # will cause circular import if using full path here

# Check whether we're in a directory where another installation of EasyVolcap exists
if (
    exists("easyvolcap/engine/__init__.py")
    and not abspath("easyvolcap/engine/__init__.py") == __file__
):
    print(
        yellow(
            f"Found an existing EasyVolcap repo at the current dir: {blue(os.getcwd())}"
        )
    )
    print(
        yellow(
            f"However, the EasyVolcap being imported is located at: {blue(dirname(dirname(dirname(__file__))))}, please be ware of this potential conflict"
        )
    )
    print(
        yellow(
            f'To use the EasyVolcap repo in the current directory, run the command: `{cyan("pip install -v -e. --no-deps --no-build-isolation")}` here'
        )
    )
...
DATASAMPLERS = Registry(
    "datasamplers"
)  # samples rays on multi-view videos # MARK: (constantly changed)

MODELS = Registry("models")  # contains model logic (rarely changed)
CAMERAS = Registry("cameras")  # contains model logic (rarely changed)
NETWORKS = Registry(
    "networks"
)  # contains most optimizable parameters # MARK: (constantly changed)
EMBEDDERS = Registry(
    "embedders"
)  # components for networks # MARK: (constantly changed)
REGRESSORS = Registry(
    "regressors"
)  # components for networks # MARK: (constantly changed)
SAMPLERS = Registry(
    "samplers"
)  # sample points on rays -> holds pointer to network # MARK: (constantly changed)
RENDERERS = Registry(
    "renderers"
)  # integrate points to rays -> holds pointer to network # MARK: (constantly changed)
SUPERVISORS = Registry(
    "supervisors"
)  # for computing loss -> holds pointer to network # MARK: (constantly changed)
...
def get_parser():
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Config file path, can be comma (not space) separated files, will inherit as ordered",
    )
    parser.add_argument(
        "-t",
        "--type",
        ...
        default="train",
        help="Execution mode, train, test or gui",
    )  # evalute, visualize, network, dataset? (only valid when run)
    parser.add_argument(
        "opts",
        action=DictAction,
        nargs=argparse.REMAINDER,
        help="dictionary like parameters (runner_cfg.resume=False), see configs/base.yaml for more options",
    )
    return parser


def update_cfg(cfg: Config):
    # Here, we define some logics fåor updating the config object
...
def parse_cfg(args):
    ...
    if "configs" in args.opts:
        if isinstance(args.opts["configs"], list):
            args.opts["configs"] += configs
        else:
            args.opts["configs"] = [args.opts["configs"]] + configs
    else:
        args.opts["configs"] = configs
    ...
    cfg = Config(
        dotdict(
            ...
            dataloader_cfg=dotdict(
                dataset_cfg=dotdict(), batch_sampler_cfg=dotdict(batch_size=1)
            ),
            runner_cfg=dotdict(
                ep_iter=500,
                epochs=400,
                visualizer_cfg=dotdict(save_tag="", result_dir=""),
                resume=True,
            ),
    ...
    if exists(args.config) and os.path.isfile(args.config):
        ...
        )  # load external configuration file (with hierarchy)
...
@catch_throw
def main():
    if not check_evc_args():
        our_args = patch_evc_args()
    parser = get_parser()
    args, argv = parser.parse_known_args()  # commandline arguments
    argv = [
        v.strip("-") for v in argv
    ]  # arguments starting with -- will not automatically go to the ops dict, need to parse them again
    argv = parser.parse_args(
        argv
    )  # the reason for -- arguments is that almost all shell completion requires a prefix for optional arguments
    args.opts.update(argv.opts)
    cfg = parse_cfg(args)
    if not check_evc_args():
        sys.argv = sys.argv[0:1] + our_args
